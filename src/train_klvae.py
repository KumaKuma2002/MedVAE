...
def setup_seg_logger(log_dir="../logs"):
    ...
    if not logger.handlers:
        fh = logging.FileHandler(os.path.join(log_dir, "seg.log"))
        fh.setFormatter(logging.Formatter("%(message)s"))
        logger.addHandler(fh)

    return logger
seg_logger = setup_seg_logger()
...
def main():
    ...
    """
    the nnunet segmenter, for organ details supervision
    """
    ...
    if args.dataset_name is not None:
        ...
        dataset["train"] = [entry.path for entry in os.scandir(args.train_data_dir)]    # All data
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model): # for accumulation steps
                ...
                """
                NOTE additional: add segmentation CE loss
                """
                b, c, h, w = pixel_values.shape
                input_image_pred_logits = get_segmentation(
                        seg_model, 
                ...
                seg_logger.info(
                    ...
                    f"recon_organs={sorted(torch.unique(target_labels).tolist())}"
                )
